...
<html lang = es-AR>
    ...
    if (isset($_POST['enviar'])) { // compruebo que se hayan enviado los datos del formulario
        ...
        if (empty($_POST['usuario_email']) || empty($_POST['usuario_clave'])) {
            ...
            $link = conectarlocal();
            $sql = mysql_query("SELECT id, name, password, email, user_type_id FROM user WHERE email='" . $usuario_email . "' AND password='" . $usuario_clave . "'");
            if ($row = mysql_fetch_array($sql)) {
                ...
                $type_user_id = $row["user_type_id"];
                $sql = mysql_query("SELECT id, name FROM user_type WHERE id='" . $type_user_id . "'");
                if ($row = mysql_fetch_array($sql)) {
