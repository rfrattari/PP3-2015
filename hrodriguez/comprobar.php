...
<?php
    if(isset($_POST['enviar'])) { // compruebo que se hayan enviado los datos del formulario
        ...
        if(empty($_POST['usuario_nombre']) || empty($_POST['usuario_clave'])) {
            ...
            $link = conectarlocal();
            $sql = mysql_query("SELECT id, name, password, email, type_user_id FROM user WHERE name='".$usuario_nombre."' AND password='".$usuario_clave."'");
            if($row = mysql_fetch_array($sql)) {
                $_SESSION['usuario_id'] = $row['id']; // creo la sesion "usuario_id" y le asignamos como valor el campo id
                $_SESSION['usuario_nombre'] = $row["name"]; // creamos la sesion "usuario_nombre" y le asignamos como valor el campo name
